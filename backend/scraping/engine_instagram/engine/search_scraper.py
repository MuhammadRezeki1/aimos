...
class InstagramSearchScraper:
    ...
    def _fetch_hashtag_via_cdp(
        ...
        def _fetch_page(tab: str, max_id: Optional[str], page_num: int):
            return page.evaluate(r"""async (params) => {
                const { tag, maxId, pageNum, tab } = params;
                try {
                    const csrf = (document.cookie.match(/csrftoken=([^;]+)/) || [])[1] || '';
            ...
            }""", {"tag": tag, "maxId": max_id, "pageNum": page_num, "tab": tab})

        def _ingest(sec: dict) -> int:
            added = 0
        ...
        # ── Tab 'top' dulu: reels (one_by_two_item.clips) banyak muncul di sini ──
        next_max_id: Optional[str] = None
        for page_num in range(1, 4):
            if len(all_posts) >= max_posts:
                break
            try:
                result = _fetch_page("top", next_max_id, page_num)
                if not result or not result.get("ok"):
                    break
                if result.get("status") == 429:
                    print(Fore.YELLOW + f"   ⚠️  429 CDP top, tunggu {rate_limit_wait}s...")
                    time.sleep(rate_limit_wait)
                    continue
                sec = result.get("data") or {}
                added = _ingest(sec)
                print(Fore.CYAN + f"   📡 CDP top page {page_num}: +{added} (total {len(all_posts)})")
                next_max_id = sec.get("next_max_id")
                if added == 0 or not next_max_id or not bool(sec.get("more_available", False)):
                    break
                time.sleep(random.uniform(1.2, 2.5))
            except Exception as e:
                print(Fore.RED + f"   ❌ CDP top error: {e}")
                break

        # ── Tab 'recent': feed terbaru (foto/video/reel inline) ──
        next_max_id = None
        page_num = 0
        ...
        while len(all_posts) < max_posts and page_num < max_pages:
            page_num += 1
            try:
                result = _fetch_page("recent", next_max_id, page_num)
                if not result or not result.get("ok"):
                    break
                if result.get("status") == 429:
                    print(Fore.YELLOW + f"   ⚠️  429 CDP recent, tunggu {rate_limit_wait}s...")
                    time.sleep(rate_limit_wait)
                    continue

                sec   = result.get("data") or {}
                added = _ingest(sec)
                print(Fore.CYAN + f"   📡 CDP recent page {page_num}: +{added} (total {len(all_posts)})")
                next_max_id = sec.get("next_max_id")
                ...
                print(Fore.RED + f"   ❌ CDP recent error: {e}")
                break
    ...
    # ════════════════════════════════════════════════════════════
    # ORCHESTRATOR — CDP & REST dulu (punya info user/owner),
    # GraphQL terakhir sebagai fallback (owner sering hanya berisi id,
    # sehingga username/full_name kosong → "Unknown account").
    # ════════════════════════════════════════════════════════════

    @staticmethod
    def _has_owner_info(posts: List[Dict]) -> bool:
        return any((p.get("owner_username") or "").strip() for p in (posts or []))
    ...
    def _collect_hashtag_posts(
        ...
        # ── Strategy 1: CDP Direct (in-browser, session-authed → owner lengkap)
        print(Fore.CYAN + f"\n   📡 [Strategy 1] CDP Direct (#{tag})")
        try:
        ...
        # ── Strategy 2: REST /api/v1/tags/ (web_info → owner lengkap) ──
        print(Fore.CYAN + f"   📡 [Strategy 2] REST /api/v1/tags/ (#{tag})")
        try:
        ...
        print(Fore.YELLOW + "   ↩️  REST kosong → coba GraphQL (fallback)...")

        # ── Strategy 3: GraphQL (fallback; owner mungkin tanpa username) ──
        print(Fore.CYAN + f"   📡 [Strategy 3] GraphQL (#{tag})")
